...
If you see the above results both data types are same and we can use them for comparision. If we compapare different values with different data types It can't be compared occuratly. In the below question I have explained clearly with python code. Please check.
...
# 1. Compare as strings
date_df['string_comparison'] = date_df['date1'] > date_df['date2']
print("--- Comparing as Strings ---")
print(date_df)
print("Result is False because '4' (from '04-...') is less than '5' (from '05-...').\n")
...
shipments_with_lateness = (
    shipments_without_lateness
    .assign(
        lateStatement=lambda df: np.where(df['is_late'], "Darn Shipment is Late", 
        "Shipment is on Time")
...
Both methods are used to achieve the same goal of filtering rows, but they do so with different syntax.
...
**Conclusion**: `.query()` is generally considered more readable for simple to moderately complex filtering. Below code can explain how those methods can be used.
...
```
If we just filter 'days_late' column like 'days_late > 5' It will give us the required results. Please check the above output. It is showing the data which shipment is late by >= 5 days

#### Briefly Give Answers to the Discussion Questions In This Section

> In this section we have learned how to filter data. We also learnt different ways of filtering. As of this section we should know how to use 'query()' method and 'loc[]' method to filter the data.
...
2.  **`filter()`**: This is like selecting required columns by passing list of column names.

**Conclusion**: I would use any of those methods based on the requirement. Let's say if I want to keep most of the columns in dataset then I would use drop method. If I want to use very few columns then I would use filter method.
...
> 1.  **`.dropna()`**: This method just drops any row if it contains a missing value in any of the columns.
    2.  **`.dropna(subset=['plannedShipDate', 'actualShipDate'])`**: This method just drops the rows contains missing value in the given columns.

Lets say you have a dataset which contains col1, col2, col3 and col4. In this case you want to use col1 and col2 as feature columns. If you use dropna without subset it just drops rows containing missing value in any of the columns.But you goal is to just drop rows contains missing value in col1 and col2. Since you are dropping many rows here your train data becomes less so you model won't be efficient.

So you should use dropna(subset=[]) as it will only drops rows which you don't want missing values in given columns, not all. So dropna(subset=[listof columns]) is more causious way of dropping rows with missing value.
:::
...
> To sort by multiple columns, you pass a list of column names, 
e.g., `.sort_values(by=['is_late', 'days_late'])`. You can also pass a list of booleans to `ascending` to specify the order for each column, like `ascending=[False, False]`. This would sort by `is_late` first (descending), and for all rows with the same `is_late` value, it would then sort them by `days_late` (descending).
>
> **Question 2: Index Management**
...
> **Question 1: Business Question Analysis**
> - **Business Question:** The analysis answers, "What percentage of our shipments contain products from more than one product category? How does grouping by `['shipID', 'prodCategory']` differ from grouping by just `'prodCategory'`?"
> 
...
Below graph shows the ontime percentage of different categories.
