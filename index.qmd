...
```
Every series created from pandas/numpy will have a property callyed 'dtype', which will give you the data type of the series.
...
**Conclusion**: I would use those methods based on the requirement,. lets say if I want to keep most of the columns in dataset then I would use drop method. If I want to use very few columns then I would use filter method.
...
Lets say when you are working with dataset and want to use few columns as feature columns for your model, if you use dropna without subset it drops many rows even your feature columns are not not having missing values and other columns has missing values. In this case you are loosing data. So dropna(subset=[]) will be more causious way of dropping columns
